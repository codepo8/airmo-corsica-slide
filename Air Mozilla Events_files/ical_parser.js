 ...
 * @version: 0.2
 *
 * Mercilessly hacked by Richard to do some time zone crossing.
 * use the link above to get Cral Saggs original code
 * The .ics files we're using are in UTC
 */
 
function pad2(n){  //function for padding .getMinutes
    return n > 9 ? "" + n: "0" + n;
} 
...
function ical_parser(feed_url, callback){
	...
	this.makeDate = function(ical_date){
		...
		dt.pacific_date = WallTime.UTCToWallTime(dt.date, "America/Los_Angeles");
		dt.eastern_date = WallTime.UTCToWallTime(dt.date, "America/New_York");
		
		//Get the full name of the given day
		dt.dayname =         days [dt.date.getDay()];
		dt.pacific_dayname = days [dt.pacific_date.getDay()];
		dt.eastern_dayname = days [dt.eastern_date.getDay()];
		dt.monthname =         months [dt.date.getMonth()];
		dt.pacific_monthname = months [dt.pacific_date.getMonth()];
		dt.eastern_monthname = months [dt.eastern_date.getMonth()];
		...
		for(var i=0;i<cal_array.length;i++){
			...
			if(in_event){
				...
				if(type =='DTSTART'){
					...
					val = dt.date;
					//var pacific_date = WallTime.UTCToWallTime(dt.date, "America/Los_Angeles");
					//var eastern_date = WallTime.UTCToWallTime(dt.date, "America/New_York");
					
					//These are helpful for display
					cur_event.date = dt.date;
					cur_event.pacific_start_time = [pad2(dt.pacific_date.getHours()),pad2(dt.pacific_date.getMinutes())].join(":");
					cur_event.eastern_start_time = [pad2(dt.eastern_date.getHours()),pad2(dt.eastern_date.getMinutes())].join(":");
					cur_event.start_time = dt.hour+':'+dt.minute;
					cur_event.start_date = dt.day+' '+dt.monthname+' '+dt.year;
					cur_event.pacific_start_date = dt.pacific_date.getDate()+' '+dt.pacific_monthname+' '+dt.year;
					cur_event.eastern_start_date = dt.eastern_date.getDate()+' '+dt.eastern_monthname+' '+dt.year;
					cur_event.day = dt.dayname;
					cur_event.pacific_day = dt.pacific_dayname
					cur_event.eastern_day = dt.eastern_dayname
				}
				...
				//Richard's hack to append multi-line event descriptions
				if (type == "") {   //continuation lines don't have a type.
					cur_event['DESCRIPTION'] += val;
				} else {
					cur_event[type] = val;
				}
	 ...
	 *
	 * hacked to return only events for the next X days
	 ^
	 * @return list of events objects
	 */
	this.getFutureEvents = function(days){
		var future_events = [], current_date = new Date(), end_date = new(Date);
		
		//Richard's hack to set the end date 
		end_date.setDate(current_date.getDate()+days);
		
		this.events.forEach(function(itm){
			//If the event starts after the current time, add it to the array to return.
			if(itm.DTSTART > current_date  && itm.DTSTART < end_date) {
